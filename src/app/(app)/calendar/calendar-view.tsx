...
import { formatMonth, shiftMonth, todayISO, weekdayOf } from "@/lib/date";
...
export function CalendarView({
  ...
  const cellCount = Math.ceil((leadingBlanks + days.length) / 7) * 7;
      ...
      <header className="flex min-w-0 flex-col gap-4 border-b border-neutral-200/80 pb-5 dark:border-neutral-800 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          <h1 className="flex items-center gap-2 text-xl font-bold tracking-tight text-neutral-900 dark:text-neutral-50 sm:text-2xl">
            <CalendarDays className="h-5 w-5 shrink-0 text-[#9E1B32] dark:text-[#e8a3b0]" />
        ...
        <div className="flex w-full items-center justify-between gap-2 rounded-lg border border-neutral-200 bg-white p-1 dark:border-neutral-800 dark:bg-neutral-900 sm:w-auto sm:justify-start">
          <button
            ...
            className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-md text-neutral-600 transition-colors hover:bg-neutral-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9E1B32] dark:text-neutral-300 dark:hover:bg-neutral-800"
      ...
      <section aria-label={`${formatMonth(month)} attendance`} className="min-w-0 overflow-hidden rounded-lg border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
        ...
        <div className="calendar-grid grid grid-cols-7 gap-px bg-neutral-200 dark:bg-neutral-800">
          {Array.from({ length: cellCount }, (_, index) => {
            const dayIndex = index - leadingBlanks;
            ...
            return (
              <Link
                key={day.date}
                href={`/attendance?date=${day.date}`}
                aria-label={`${formatMonth(month)} ${dayNumber}: ${accessibleSummary}. Open attendance.`}
                className="calendar-cell group flex min-h-14 min-w-0 flex-col items-center bg-white text-center transition-colors hover:bg-neutral-50 focus-visible:z-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#9E1B32] dark:bg-neutral-900 dark:hover:bg-neutral-800/70 sm:min-h-24 sm:items-stretch sm:p-2 sm:text-left lg:min-h-32 lg:p-3"
              >
                <span className={`inline-flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold sm:h-8 sm:w-8 sm:text-sm ${isToday ? "bg-[#9E1B32] text-white" : "text-neutral-700 group-hover:text-[#9E1B32] dark:text-neutral-300 dark:group-hover:text-[#e8a3b0]"}`}>
                  {dayNumber}
                </span>
