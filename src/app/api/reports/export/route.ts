...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const filenameDate = [summary.from, summary.to, todayISO()]
      .map((date) => formatDate(date).replace(/[^\p{L}\p{N}]+/gu, "-"))
      .join("-to-");
    const filename = `attendance-${batch.name.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-${filenameDate}.csv`;
